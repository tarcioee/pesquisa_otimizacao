...
int main(int argc, char* argv[]) {
  ...
  for(int i =0; i<argc; i++){
    ...
    if(teste == "--neighborhood_size"){
      T_neighborhood_size = stof(argv[i+1]);
    }
  }
    //Testes MOEAD
      BoundedParetoSet *EP = new BoundedParetoSet(); //ver se preciso usar new
      Population population;
      ...
      population = MOEAD(saved_generations, EP, tempoTotal,instance, initial_population, max_valuations_after_alg, N_subproblems, T_neighborhood_size, weight_vectors, mutation_chance, crossover_chance, valuations_before_alg );
